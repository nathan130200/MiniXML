...
public static class Xml
{
    ...
    static readonly Comparer<string> s_AttributeSorter = Comparer<string>.Create(AttributeSorterImpl);

    static int AttributeSorterImpl(string lhs, string rhs)
    {
        if (lhs == null)
            return 1;
        else if (rhs == null)
        ...
        if (lhs.StartsWith("xmlns:") || lhs.StartsWith("xml:"))
        {
            if (rhs.StartsWith("xmlns:") || rhs.StartsWith("xml:"))
                return 0;
    ...
    {
        var entries = e.Attributes.Where(IsXmlOrXmlnsDeclaration).OrderBy(x => x.Key, s_AttributeSorter);
